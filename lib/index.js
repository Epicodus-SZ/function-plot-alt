...
// This is stuff that doesn't move
  ...
  Chart.prototype.buildAxis = function() {
    ...
    // update
    // This section altered by STZ on 10/25/2007
    // This is creating the origin placement of the chart items
  ...
  Chart.prototype.buildContent = function() {
    ...
    if (options.xAxis.type === 'linear') {
      ...
      var newSteve = content.selectAll(':scope > g.steveX')
      .data([
        [
          [0, yScale.domain()[0]],
          [0, yScale.domain()[1]]
      ...
      newSteve.enter()
        .append('g')
        .attr('class', 'steveX')
      newSteve.attr('d', line)
      newSteve.exit()

      var mover = content.select('x.axis')
      var removed = mover.remove()
      newSteve.append(function() {
        return removed.node();
      });
      ...
      // var szTest = canvas.selectAll(':scope > g.x.axis')
      //   .data([
      //     [
      //       [xScale.domain()[0], 0],
      //       [0, yScale.domain()[1]]
  ...
  // This section updates the ticks range on the
